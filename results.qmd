...
nhanes_ridge <- nhanes_women |>
  ...
  mutate(
    preg_status = case_when(
      ever_pregnant == 1 ~ "Ever pregnant",
      ever_pregnant == 2 ~ "Never pregnant",
      TRUE ~ NA_character_
    )
  ) |>
  filter(!is.na(preg_status))

ggplot(nhanes_ridge, aes(x = sedentary_min_day, y = preg_status, fill = preg_status)) +
  ...
  labs(
    x = "Sedentary Minutes per Day",
    y = "",
    title = "Sedentary Minutes Distribution (Ages 20–40) by Pregnancy History and NHANES Cycle"
  ) +
  theme_minimal() +
  theme(
    legend.position = "none",
